...
use App\Services\MailboxNotificacionService;
...
class MailboxController extends Controller
{
    ...
    public function iniciarTicket(int $workorderId)
    {
        $localUserId = auth()->id();
        $identityId  = UserFirebirdIdentity::where('firebird_user_clave', $localUserId)->value('id');
        ...
        $workorder->status_id = 5; // En proceso
        $workorder->save();
        ...
        // 🔥 Notificar por WhatsApp a todos los involucrados
        (new MailboxNotificacionService())
            ->notificarTicketIniciado($workorder, $identityId);

        return response()->json($workorder);
    }

    public function FinalizarTicket(int $workorderId)
    {
        $localUserId = auth()->id();
        $identityId  = UserFirebirdIdentity::where('firebird_user_clave', $localUserId)->value('id');
        ...
        $workorder = WorkOrder::findOrFail($workorderId);
        $workorder->status_id = 7; // En proceso
        $workorder->save();
        ...
        // 🔥 Notificar por WhatsApp a todos los involucrados
        (new MailboxNotificacionService())
            ->notificarTicketFinalizado($workorder, $identityId);
